...
def test_full_cartesian_product_is_exactly_six_tuples() -> None:
    """Expanding the external axes yields exactly the six verifiable supported tuples (D-35)."""
...
def test_current_core_sits_inside_declared_ranges() -> None:
    """The current core version and every external value lie within the published D-29 ranges."""
...
def test_ci_derives_core_and_references_the_manifest() -> None:
    ...
    assert "pyproject.toml" in ci, "CI must derive the core version from pyproject.toml"
