"""Unit tests for CKAN resource.access + resource.schema population (D-P5-02/03/04).
...
from datasluice.connectors.ckan.mapper import map_resource
from datasluice.domain import HttpDownload, QueryAccess


def test_access_httpdownload_when_url_set() -> None:
    resource = map_resource({"id": "r1", "url": "https://example.com/data.csv", "format": "CSV"})
...
def test_access_queryaccess_when_no_url_and_datastore_active() -> None:
    resource = map_resource({"id": "r1", "url": None, "datastore_active": True, "datastore_fields": [{"id": "col1"}]})
...
def test_schema_from_schema_fields_fallback() -> None:
    resource = map_resource({"id": "r1", "url": "https://x", "schema": {"fields": [{"id": "colA"}]}})
    assert resource.schema is not None
    assert resource.schema.columns == [{"id": "colA"}]


def test_schema_none_when_portal_silent() -> None:
    resource = map_resource({"id": "r1", "url": "https://x"})
