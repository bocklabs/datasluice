...
def test_row44_delete_dataset_returns_redacted_receipt() -> None:
    ...
    transport, client = _sync_client_with_transport(routes, UDataCredential(api_key="secret-key"))
    with client:
        outcome = client.datasets.delete("abc", _USER_PERMISSIONS, DatasetDeleteOptions(send_legal_notice=True))
...
def test_rows78_79_extras_read_and_null_delete_semantics() -> None:
    ...
    with client:
        ...
        extras = client.datasets.update_extras_v2(
...
def test_row80_extras_delete_returns_receipt_from_204_with_body() -> None:
    ...
    with client:
        outcome = client.datasets.delete_extras_v2(
            "abc", DatasetExtrasDelete(keys=("gone",)), permissions=_USER_PERMISSIONS
        )
...
def test_async_dataset_service_matches_sync_wire_exactly() -> None:
    ...
    async def run() -> None:
        async with client as active:
            ...
            record = await active.datasets.create(
                DatasetCreateInput(title="T", description="D"), permissions=_USER_PERMISSIONS
            )
            outcome = await active.datasets.delete("abc", permissions=_USER_PERMISSIONS)
            assert len(envelope.items) == 1
            assert record.id.value == "new"
            assert outcome.status_code == 204
    ...
    with sync_client:
        ...
        sync_client.datasets.create(DatasetCreateInput(title="T", description="D"), permissions=_USER_PERMISSIONS)
        sync_client.datasets.delete("abc", permissions=_USER_PERMISSIONS)
...
def test_cr03_destructive_calls_are_never_auto_retried() -> None:
    ...
    with client:
        with pytest.raises(CatalogUnavailableError):
            client.datasets.delete("abc", _USER_PERMISSIONS)
...
def test_cr05_configured_probe_runner_is_used_for_effective_evidence() -> None:
    ...
    routes = _site_first({("GET", "http://127.0.0.1:5640/api/1/datasets/?page=1&page_size=20"): _page_body()})
    transport = RouterTransport(routes)
    client = SyncUDataClient(
        transport,
        declared_udata_profile(),
