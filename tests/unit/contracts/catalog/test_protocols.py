...
def test_normalized_services_use_operation_requests_guards_and_typed_envelopes() -> None:
    ...
    assert "operation" in inspect.signature(SyncDatasetService.get).parameters
...
def test_client_protocols_are_explicit_context_managers_with_matching_services() -> None:
    ...
    async_services = {
        name for name in ("datasets", "resources", "organizations") if name in AsyncCatalogClient.__dict__
    }
...
def test_connector_context_keeps_sync_and_async_executors_independent() -> None:
    ...
    assert isinstance(context.credentials, CKANCredential)
...
def test_public_protocols_have_no_transport_escape_hatch() -> None:
    ...
    assert all(
        forbidden.isdisjoint(set(get_type_hints(protocol).keys()) | set(protocol.__dict__)) for protocol in protocols
    )
