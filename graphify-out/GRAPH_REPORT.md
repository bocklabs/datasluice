...
## Corpus Check
- 130 files · ~42,661 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1567 nodes · 3046 edges · 102 communities (81 shown, 21 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 145 edges (avg confidence: 0.64)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9a185492`
...
Cohesion: 0.06
Nodes (50): Any, RLock, datasluice_source(), mirror_dlt_state(), dlt (data load tool) integration: use DataSluice as a dlt source. Requires…, Mirror dlt's load-committed per-resource watermarks into a DataSluice…, Return a deterministic destination-safe name for a resource ID., Return a dlt source yielding one Arrow-backed table per resource. Args: portal:… (+42 more)
...
### Community 5 - "Any"
Cohesion: 0.18
Nodes (12): _chain(), _close_source(), _content_encoding_from_headers(), Any, Open an already-fetched streaming response through the data plane., Dispatch to the format reader and wrap output in BatchStream., Extract a lowercased Content-Encoding value from response headers., Strip the fsspec storage scheme from *uri* to produce the path component. (+4 more)
...
Cohesion: 0.15
Nodes (18): Fetch organization metadata via ``/organizations/{slug}/``., map_dataset(), map_license(), map_organization(), map_resource(), Any, Mapping functions to convert data.gouv.fr (udata) JSON into domain models., Convert a udata license dict into a :class:`License`. (+10 more)
...
Cohesion: 0.10
Nodes (18): APIKeyAuth, Any, API-key authentication strategy. Supports passing the key via a header (default…, Authenticate requests using an API key. Args: api_key: The API key value.…, BaseAuth, ABC, Abstract base for authentication strategies., Protocol for pluggable authentication. Each strategy knows how to decorate a… (+10 more)
...
Cohesion: 0.10
Nodes (17): CachePort, Protocol, Cache port Protocol for content-addressed byte caching., Boundary protocol for a simple key/byte cache., PortalDetector, Protocol, Portal detector port Protocol., Detection seam protocol returning evidence-based portal identification. (+9 more)
...
Cohesion: 0.08
Nodes (27): NoAuth, Any, Authentication strategy that adds no credentials. Suitable for portals that are…, Default configuration constants., Configuration for DataSluice., create_default_transport(), Default transport factory for the DataSluiceSession composition root. Replaces…, Construct a default transport from ``DEFAULT_*`` constants. Picks… (+19 more)
...
### Community 82 - "CredentialAwareRedirectHandler"
Cohesion: 0.19
Nodes (9): HTTPMessage, Request, CredentialScope, Credential scoping model for host-bound credential policy., Host-scoped policy controlling where credentials may be sent. Attributes:…, CredentialAwareRedirectHandler, Redirect handling that strips credentials on cross-origin or scheme-downgrade…, Strip sensitive headers when a redirect crosses origins or downgrades to plain… (+1 more)
...
Cohesion: 0.07
Nodes (50): map_dataset(), map_license(), map_organization(), map_resource(), Any, Mapping functions to convert CKAN-native JSON into domain models., Convert a CKAN package dict into a :class:`Dataset`. ``base_url`` is forwarded…, Convert a CKAN license dict into a :class:`License`. (+42 more)
...
### Community 122 - "SearchableCatalog"
Cohesion: 0.28
Nodes (7): CatalogPort, OrganizationCatalog, Protocol, Marker base protocol all catalog connectors share. Attributes: portal_type:…, Capability protocol for dataset search., Capability protocol for organization lookup., SearchableCatalog
